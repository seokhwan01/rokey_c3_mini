...
class FollowingCar(Node):
    def __init__(self):
        ...
        self.model = YOLO("/home/rokey/rokey3_C1_ws/src/detection/detection/model/real_final_best.pt")
    ...
    def send_goal(self):
        x, y, z = self.latest_map_point.point.x, self.latest_map_point.point.y, self.latest_map_point.point.z
        ...
        if not (-2.7 <= x_offset <= 0.3 and -0.6 <= y_offset <= 3.0):
            self.get_logger().warn(f"Filtered out: map coords out of bounds (x={x_offset:.2f}, y={y_offset:.2f})")
            return
        ...
        self.get_logger().info(f"Sending goal to: ({x_offset:.2f}, {y_offset:.2f})")
