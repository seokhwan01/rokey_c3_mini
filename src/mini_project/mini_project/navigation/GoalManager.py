...
class GoalManager(Node):
    ...
    def qr_response_callback(self, future):
        try:
            ...
            if response.success:
                ...
                self.call_follow_service() # 큐알인식이 되었으니 팔로우 중지하라는 서비스 요청을 보냄
    ...
    def call_follow_service(self):
        ...
        future = self.follow_service_client.call_async(request)
        future.add_done_callback(self.follow_response_callback)

    def follow_response_callback(self, future):
        response = future.result()
        if response.success:
            self.get_logger().info(f'Following state: {response.message}')# response.message = "FollowingCar stopped."

            # === JSON 파일 경로 설정 ===
            file_path = f'/home/rokey/rokey_c3_mini/src/mini_project/map/best_{self.qr_id}.json'
            #self.qr_id를 기반으로 QR 코드에 대응되는 goal 경로가 저장된 JSON 파일
